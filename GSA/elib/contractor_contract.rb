...
#
  DB_CONNECT.run('CREATE TABLE IF NOT EXISTS ElibMain_contractor_list
	(
	...
	schedule_start = "#{ELIBMAIN}#{SCHEDULE_LIST}?"
...
5.times do
		threads << Thread.new do
			until @l_queue.empty?
				html      = send_agent("#{ELIBMAIN}#{CONTRACTOR_LIST}#{@l_queue.pop}&executeQuery=YES")

...
def create_client_table(name, extra_columns = [])
	...
	DB_CONNECT.create_table! name do
		primary_key :id
...
def find_duplicates
	DB_CONNECT[:manufacture_parts].order(:last_updated).distinct(:mfr, :mpn)
	DB_CONNECT[:search_manufactures].group_and_count(:mfr, :mpn)
end
...
def prioritize(table)
	...
	manufacture_count.each do |mfr|
		...
		affected = DB_CONNECT[:search_manufactures].where(:name, similar).update(priority: priority, check_out: 0)
		...
		if affected > 0
			...
			affected = DB_CONNECT[:search_manufactures].where(Sequel.ilike(:name, similar)).update(priority: priority, check_out: 0)
